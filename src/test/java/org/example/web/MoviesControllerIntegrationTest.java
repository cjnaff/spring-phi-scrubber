...
class MoviesControllerIntegrationTest {
    ...
    @Test
    void shouldGetMovieByTitleName() {
        //given
        MovieDTO movieToRetrieve = GONE_WITH_THE_WIND;
        String movieTitle = movieToRetrieve.getTitle();

        //when
//        ResponseEntity<MovieDTO> response = getMovieByTitleName(movieTitle);
        MovieDTO responseMovie =  getMovieByTitleName(movieTitle);
...
//        assertMovieRetrieved(response, movieToRetrieve);
    ...
    @Test
    @DirtiesContext
    void shouldFailCreateMovieWithNullName() {
        //given
        var movieToCreate = new MovieDTO(null, 2024);
    ...
    private MovieDTO getMovieByTitleName(String name) {
        return restTemplate.getForObject(getMovieNameUrl(name), MovieDTO.class);
    }
    ...
    private String getMovieNameUrl(String name) {
        return String.format(getMoviesApiUrl() + "/name/%s",  name );
    }
    ...
    private void assertInternalServerError(ResponseEntity<MovieDTO> response) {
        assertThat(response.getStatusCode())
                .isEqualTo(HttpStatus.INTERNAL_SERVER_ERROR);
    }
